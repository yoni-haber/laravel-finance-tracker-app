<div class="space-y-6">
    <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form wire:submit.prevent="save" class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div>
                <label
                    class="block text-xs font-semibold uppercase tracking-wide text-gray-900 dark:text-white">Type</label>
            ...
            <div>
                <label class="block text-xs font-semibold uppercase tracking-wide text-gray-900 dark:text-white">Amount
                    (£)</label>
            ...
            <div>
                <label
                    class="block text-xs font-semibold uppercase tracking-wide text-gray-900 dark:text-white">Date</label>
            ...
            <div class="flex items-center gap-3">
                <label class="inline-flex items-center gap-2">
                    <span
                        class="text-xs font-semibold uppercase tracking-wide text-gray-900 dark:text-white">Recurring</span>
                    <input type="checkbox" wire:model.live="is_recurring"
                           class="rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500"/>
                </label>
                <select wire:model.live="frequency"
                        class="rounded-md border-gray-300 dark:bg-zinc-800 dark:border-zinc-700 text-sm" @disabled(! $is_recurring)>
            ...
            @if ($is_recurring)
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wide text-gray-900 dark:text-white">Recurring
                    ...
                    <input
                        ...
                        class="mt-2 w-full rounded-md border-gray-300 dark:bg-zinc-800 dark:border-zinc-700"
                    />
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Set an end date before creating a new
                        recurring amount to keep historical totals intact.</p>
            ...
            <div class="md:col-span-2 lg:col-span-3 flex items-center gap-3">
                <button type="submit" class="rounded-md bg-emerald-600 px-4 py-2 text-white hover:bg-emerald-700">Save
                </button>
                @if ($transactionId)
                    <button type="button" wire:click="resetForm"
                            class="rounded-md border border-zinc-300 px-4 py-2 text-sm">Cancel
                    </button>
                @endif
        ...
        <div class="flex flex-wrap gap-4 items-center justify-between">
            ...
            <div class="flex gap-2 text-sm">
                ...
                <input type="number" wire:model.live="year"
                       class="w-24 rounded-md border-gray-300 dark:bg-zinc-800 dark:border-zinc-700" min="2000"
                       max="2100"/>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 text-sm">
                ...
                @forelse ($transactions as $transaction)
                    <tr>
                        ...
                        <td class="px-3 py-2">
                            <span
                                class="rounded-full px-2 py-1 text-xs {{ $transaction->type === \App\Models\Transaction::TYPE_INCOME ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">{{ ucfirst($transaction->type) }}</span>
                            @if ($transaction->is_recurring)
                                <span class="ml-2 text-xs text-emerald-600">Recurring ({{ ucfirst($transaction->frequency) }})</span>
                            @endif
                        </td>
                        <td class="px-3 py-2 font-semibold {{ $transaction->type === \App\Models\Transaction::TYPE_INCOME ? 'text-emerald-600' : 'text-rose-600' }}">
                            £{{ number_format($transaction->amount, 2) }}</td>
                        <td class="px-3 py-2">{{ $transaction->description }}</td>
                        <td class="px-3 py-2 text-right space-x-2">
                            <button type="button" wire:click="edit({{ $transaction->id }})"
                                    class="text-sm text-blue-600">Edit
                            </button>
                            <button
                                type="button"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-3 py-4 text-center text-sm text-gray-500">No transactions found for
                            this period.
                        </td>
                    </tr>
                @endforelse
